...
# 组合模式：显式消费 al-sandbox-skill 生成的一次性 handoff
...
- 组合模式：先由 `al-sandbox-skill handoff --output <file>` 请求 Sandbox MCP 签发短期、一次性、owner-bound 的 SourceBundle export grant，再由 `save-current --handoff @<file>` 显式消费。`save-current` 强制要求 handoff，不保留隐式 conversation 切换。Site 随后把产物落为自己的不可变 OCI SourceBundle，并独立完成生产构建和发布。
...
- `save-current` 只接受 `al-sandbox-skill handoff` 产生的显式一次性描述符；Site MCP 不配置长期 Sandbox endpoint/token，也没有隐式 `current_conversation` source。local、Git 和 OCI 路径完全独立于 Sandbox。
