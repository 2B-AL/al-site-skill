...
- `DeleteSite` 是永久删除操作，必须由用户明确要求，并同时传 `confirm=true`、最新 `expected_uid` 和 `resource_version`。
...
python3 scripts/al_mcp.py sites                         # 默认：由当前用户创建
python3 scripts/al_mcp.py sites --relation accessible  # 当前 owner 成员关系允许管理
python3 scripts/al_mcp.py sites --owner-kind team --owner-id team-123 --phase Ready
python3 scripts/al_mcp.py get SITE_ID --relation accessible
```

`created` 和 `accessible` 是两种独立关系。`created` 依据创建时持久化且产品 API 不可修改的创建者身份，
因此也包含当前用户创建但 owner 为 team/org 的 Site；`accessible` 依据当前
user/team/org owner 成员关系，表示调用者拥有 MCP 控制面管理权限。Site 的 public/selected audience、
公网可访问链接或应用侧身份验证只属于数据面，不会让资源出现在 `accessible` 查询中。不要把
accessible 结果表述为“都是我创建的”。List/Get 统一返回 creator、owner、relations、permissions、
status、UID、resource version、时间和 details；更新和删除前必须使用刚读取到的 resource version。
...
- 端到端测试使用 `test-deploy-local` / `test-deploy-current`；它们只创建专用测试 Site，并持续记录不含 token/source receipt 的 `0600` run manifest。`cleanup-test-run` 必须同时匹配 Site name、UID 并使用 GetSite 返回的最新 resource version 才调用删除，禁止用名称前缀扫描或清理历史资源。
